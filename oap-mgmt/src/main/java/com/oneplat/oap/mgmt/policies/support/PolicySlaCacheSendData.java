 ...
 * @date 2015. 7. 21
 */
public class PolicySlaCacheSendData implements CacheSendData {
    private ServicePolicy data;
    private Environment env;

    public PolicySlaCacheSendData(ServicePolicy data, Environment env) {
        super();
        this.data = data;
        this.env = env;
    }
    ...
    public RestSendData makeDeleteData() throws Exception {
     // TODO Auto-generated method stub
        ...
        if(data.getApiGroupNumber()==0)
            url = env.getProperty("interface.server.domain") + InterfaceConstants.POLICY_SLA_DATA_URL + data.getServiceNumber();
        else
            url = env.getProperty("interface.server.domain") + InterfaceConstants.POLICY_SLA_DATA_URL + data.getApiGroupNumber();
        /* restSendData set*/
    ...
    public Map<String,Object> convertData(){
        ...
        parameterMap.put("testPeriod", InterfaceSenderUtil.getIntervalStringValue(data.getTestCriteriaCode()));
